...
//		}
		...
		for( int X = 0; X < 20; X++ ) {
			if (X==19) {
				Body = new PhysicsBody(world, 100f); //Create a few triangles. Only boxes would get boring, right?
			} else
			{
				Body = new PhysicsBody(world, 1f); //Create a few triangles. Only boxes would get boring, right?
			}
			Vertex V2 = new Vertex( world, Body, 100 , 12+X*10 );
			if (X==0) {
				V1 = new Vertex( world, Body, 100 , 10 );
			}		
	...
	public void run() {
		...
		do {
        	...
        	if( dragVertex == null ) {
    			dragVertex = world.findVertex(100, 10);
        	}
       		
//    		dragVertex.position.x += (mouseX-dragVertex.position.x)/10;
//    		dragVertex.position.y += (mouseY-dragVertex.position.y)/10;
